...
export const ExactCadInteractiveBlueprint: React.FC<ExactCadInteractiveBlueprintProps> = ({
  ...
  const getPlotByNum = (num: number): Plot => {
    ...
    const padded2 = `P-${String(num).padStart(2, '0')}`;
    const direct = `P-${num}`;
    return (
      plots.find((p) => p.plotNumber === padded3 || p.plotNumber === padded2 || p.plotNumber === direct) || {
  ...
  const isSelected = (num: number) => {
    ...
    const direct = `P-${num}`;
    return (
      selectedPlotNumber === padded3 ||
      selectedPlotNumber === padded2 ||
