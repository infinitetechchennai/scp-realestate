...
import { cadSurveyDimensions } from '../data/mockTownshipPlots';
...
export function parsePlotCsv(csvContent: string, projectName = 'SCP Farm Layout (184 Plots)'): Plot[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const cadMeta = cadSurveyDimensions[numericId];

    const area = parseFloat(sizeIdx !== -1 ? row[sizeIdx] : row[1]) || cadMeta?.area || 1500;
    ...
    const roadWidth = cadMeta?.roadWidth || '20 ft';
